"""
Script para testear el flujo de completar_tarea por WebSocket
Simula un reloj enviando el comando para marcar una tarea como completada
"""
...
async def test_completar_tarea():
    """Prueba completar una tarea por WebSocket"""
    
    # Parámetros de prueba
    WS_URL = "ws://localhost:8000/ws"
    UUID_RELOJ = "test-reloj-001"
    
    print("="*60)
    print("TEST WEBSOCKET - COMPLETAR TAREA")
    print("="*60)
    print(f"URL: {WS_URL}")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.ws_connect(WS_URL, protocols=['arduino', 'web-client']) as ws:
                ...
                # Mensaje para completar la tarea con id=1
                mensaje_completar = {
                    "tipo": "tareas",                   # IMPORTANTE: "tareas" no "relojes"
                    "comando": "completar_tarea",
                    "tarea": {
                        "id": 1,                    # ID de la tarea en historial
                        "id_empleado": 1,           # ID del empleado que la completa
                        "tipo": "normal"            # Tipo de tarea (normal o extra)
                    },
                    "uuid": UUID_RELOJ
                }
                ...
                # Esperar respuesta (con timeout de 10 segundos esta vez)
                try:
                    ...
                    print("\nPosibles causas:")
                    print("1. El comando 'completar_tarea' no llegó a Tareas.actualizar_tarea()")
...
if __name__ == "__main__":
    ...
    asyncio.run(test_completar_tarea())
