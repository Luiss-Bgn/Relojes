...
// Helpers de usuario/rol reutilizables
function getLoggedUser() {
  const raw = localStorage.getItem('loggedUser');
  if (!raw) return { role: 'visitante' };
  try {
    const parsed = JSON.parse(raw);
    return parsed && typeof parsed === 'object' ? parsed : { role: 'visitante' };
  } catch {
    return { role: 'visitante' };
  }
}

function filterTrabajadoresForUser(allEmpleados = [], loggedUser = { role: 'visitante' }) {
  const userRole = (loggedUser.role || 'visitante').toLowerCase();
  return allEmpleados.filter((emp) => {
    const empRole = (emp.role || emp.role_dp || 'empleado').toLowerCase();

    // Nunca mostrar administradores
    if (empRole === 'admin' || empRole === 'administrador') return false;
...
async function init() {
  ...
  try {
    ...
    state.trabajadores = filterTrabajadoresForUser(allEmpleados, loggedUser);
...
async function checkForUpdates() {
  ...
  if (empleadosData) {
    const loggedUser = getLoggedUser();
    state.trabajadores = filterTrabajadoresForUser(empleadosData, loggedUser);
