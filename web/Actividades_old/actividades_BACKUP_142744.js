...
const state = {
  ...
  daysOffset: 0, // [NUEVO] Offset para carrusel de 14 días (0-13)
  lastLoadedWeekStart: null, // 🔥 NUEVO: Rastrear qué semana está cargada (fecha inicio)
  trabajadores: [],
  currentEmpPage: 0,
 ...
 * Calcula la fecha de inicio de semana (lunes) para una fecha dada
 * Retorna string en formato YYYY-MM-DD
 * IMPORTANTE: Debe estar definido antes de init() para ser usado
 */
function getWeekStartForDate(date) {
  const d = new Date(date);
  d.setHours(0, 0, 0, 0);
  // JavaScript: 0=domingo, 1=lunes, etc.
...
async function init() {
  ...
  try {
    ...
    // 🔥 NUEVO: Guardar la semana actual como la semana cargada
    state.lastLoadedWeekStart = getWeekStartForDate(new Date());
    console.log(`📅 [init] Semana inicial cargada: ${state.lastLoadedWeekStart}`);
  ...
  // 🔥 OPTIMIZADO: Reducir de 1000ms a 10000ms para evitar parpadeo excesivo de la tabla
  startAutoRefresh(10000);

  // 🔥 NUEVO: Verificar cada minuto si cambió el día y recargar automáticamente
  let lastDate = new Date().getDate();
  setInterval(() => {
    const now = new Date();
    const currentDate = now.getDate();
    if (currentDate !== lastDate) {
      console.log(`📅 Cambio de día detectado: ${lastDate} → ${currentDate}. Recargando página...`);
      lastDate = currentDate;
      location.reload(); // Recarga la página para mostrar el nuevo día
    }
  }, 60000); // Verificar cada 60 segundos (1 minuto)

  // 🔥 REMOVIDO: Este intervalo causaba parpadeo visual cada 30 segundos
  // Las tareas extra ya se actualizan vía WebSocket (tiempo real) y checkForUpdates() (cada 10 seg)
  // setInterval(async () => {
  //   await refreshAvailableExtraTasks(true);
  //   renderForCurrentState(); // Esto causaba el brinco visual
  // }, 30000);
    ...
    webSocketConnection.onmessage = async (event) => {
      try {
        ...
        if (mensaje.accion === 'tarea_extra_completada') {
          ...
          showToast(`Tarea completada por ${mensaje.empleado}. Recargando...`, 'success', 2000);
          
          // 🔥 HACER UN RELOAD COMPLETO PARA LIMPIAR CACHE Y BUGEOS
          // Esperar un poco para que el usuario vea el toast, luego recargar
          setTimeout(() => {
            console.log('🔄 Recargando página completa para limpiar cache...');
            location.reload();
          }, 2000);
        }
      } catch (err) {
...
async function recargarDatosDelServidor() {
  try {
    ...
    // 🔥 CRÍTICO: Cargar extras ANTES de construir cache y renderizar
    await cargarTareasExtrasCompletadas();
...
async function refreshAvailableExtraTasks(forceCheck = false) {
  ...
  tareas.forEach(tarea => {
    ...
    for (const trab of state.trabajadores) {
      const tareasDelTrab = (trab.tareas_asignadas && trab.tareas_asignadas[dayName]) || [];
    ...
    yaCompletadaComoExtra = state.trabajadores.some(emp => {
      const tareasEmp = (emp.tareas_asignadas && emp.tareas_asignadas[dayName]) || [];
...
async function checkForUpdates() {
  ...
  if (empleadosData) {
    ...
    if (loggedUser.role === 'empleado' || loggedUser.role === 'visitante') {
      // Empleados y visitantes NO ven supervisores ni admins
      state.trabajadores = empleadosData.filter(emp => {
        const empRole = emp.role ? emp.role.toLowerCase() : (emp.role_dp ? emp.role_dp.toLowerCase() : 'empleado');
        return empRole !== 'admin';  // Nunca mostrar admin
      });
    } else {
      // Admin y supervisor TAMBIÉN excluyen al admin de la tabla
      state.trabajadores = empleadosData.filter(emp => {
        const empRole = emp.role ? emp.role.toLowerCase() : (emp.role_dp ? emp.role_dp.toLowerCase() : 'empleado');
        return empRole !== 'admin';  // Nunca mostrar admin
      });
    ...
    // 🔥 REMOVIDO: updateCellStates() ya se ejecuta en updateTime() cuando cambia el minuto
    // Evita parpadeos innecesarios de la tabla
  }
...
function startAutoRefresh(intervalMs = 1000) {
  ...
  window.checkForUpdates = checkForUpdates; // 🔥 NUEVO: Exponemos checkForUpdates globalmente
}
...
function cacheSelectors() {
  ...
  DOM.prevDayBtn = document.getElementById('prev-day-btn'); // NUEVO: botón día anterior
...
function bindUIEvents() {
  // 👉 Botón siguiente día (MODIFICADO: sin límite, permite navegar hacia el futuro)
  DOM.nextDayBtn?.addEventListener('click', () => {
    animateDayChange('left', () => {
      state.daysOffset = state.daysOffset + 1; // Sin límite, avanza al futuro
      renderForCurrentState();
      DOM.tableWrapper.scrollTop = 0;
    });
  });

  // 👉 Botón día anterior (NUEVO: permite retroceder días)
  DOM.prevDayBtn?.addEventListener('click', () => {
    // Solo permitir retroceder si no estamos en el día actual
    if (state.daysOffset > 0) {
      animateDayChange('right', () => {
        state.daysOffset = state.daysOffset - 1;
        renderForCurrentState();
        DOM.tableWrapper.scrollTop = 0;
      });
    }
  ...
  // 👉 Botón ir a hoy (MODIFICADO: resetea daysOffset)
  DOM.todayBtn?.addEventListener('click', () => {
    animateDayChange('right', () => {
      state.currentDayIndex = new Date().getDay();
      state.daysOffset = 0; // Reset del offset para volver a hoy
      renderForCurrentState();
      DOM.tableWrapper.scrollTop = 0;
    });
  });
  ...
  DOM.tbody?.addEventListener('click', (e) => {
    ...
    if (td.cellIndex === 0 || td.cellIndex === 1 || td.cellIndex === 2) {
      console.log('🖱️ Click en columna editableeeee:', td.cellIndex);
      console.log('Llo hareeeeeeeeeeee');
      ...
      // Obtener información de todas las celdas de la fila
      const horarioCell = row.cells[0];
      const actividadCell = row.cells[1];
      const puntosCell = row.cells[2];
      
      // Extraer hora actual (puede estar en formato "HH:MM - HH:MM" o "HH:MM hrs")
      let horaInicio = horarioCell.textContent.trim();
      let horaFin = '';
      
      if (horaInicio.includes(' - ')) {
        [horaInicio, horaFin] = horaInicio.split(' - ').map(h => h.trim());
      } else if (horaInicio.includes(' hrs')) {
        horaInicio = horaInicio.replace(' hrs', '').trim();
      }
      ...
      let puntosActividad = puntosCell.textContent.trim();
      // Convertir "-" a 0
      if (puntosActividad === '-' || puntosActividad === '') {
        puntosActividad = '0';
      }
      ...
      // Abrir modal de edición para la actividad completa
      // Necesitamos encontrar un empleado que tenga esta tarea para obtener empId y tareaId
      const dayName = diasSemana[state.currentDayIndex];
      
      // Buscar el primer empleado que tenga esta tarea
      let tareaInfo = null;
      for (const empleado of state.trabajadores) {
        ...
        const tarea = tareas.find(t => {
          ...
          const nombreMatch = nombreTarea.toLowerCase().replace(/\s+/g, ' ') === nombreActividad.toLowerCase().replace(/\s+/g, ' ');
          const horaMatch = horaTarea === horaInicio;
          
          return nombreMatch && horaMatch;
        });
        ...
        if (tarea) {
          ...
          tareaInfo = {
            ...
            tareaId: tarea.id,
            nombre: nombreActividad,
            descripcion: descripcionActividad || tarea.descripcion || '',
            hora: horaInicio,
            hora_fin: horaFin || tarea.hora_fin || '',
            puntaje: tarea.puntaje || puntosActividad,
      ...
      if (tareaInfo) {
        // 🔥 Agregar flag para indicar que es edición global (todos los empleados)
        tareaInfo.editarTodos = true;
        openModal(tareaInfo);
      } else {
        console.log("te voy a encontrar perra")
        console.error('❌ No se encontró la tarea. Datos buscados:', {
          hora: horaInicio,
          nombre: nombreActividad,
          dia: dayName,
          totalEmpleados: state.trabajadores.length
        });
    ...
    if (loggedUser && loggedUser.role && (loggedUser.role === 'admin' || loggedUser.role === 'supervisor')) {
      ...
      const tareaId = td.dataset.tareaId ? Number(td.dataset.tareaId) : null;
...
/**
 * 🔥 NUEVO: reloadDataForWeek(fecha)
 * Recarga los datos de empleados y tareas para una semana específica
 * @param {string} fecha - Fecha en formato YYYY-MM-DD
 */
async function reloadDataForWeek(fecha) {
  console.log(`🔄 [reloadDataForWeek] Recargando datos para semana de: ${fecha}`);
  
  try {
    const resp = await fetch(`/empleados-con-tareas?fecha=${fecha}`);
    if (!resp.ok) throw new Error('Error al obtener los empleados');
    let allEmpleados = await resp.json();
    
    // 🔥 Filtrar empleados según el rol del usuario logueado (mismo filtro que init)
    const loggedUserString = localStorage.getItem('loggedUser');
    const loggedUser = loggedUserString ? JSON.parse(loggedUserString) : { role: 'visitante' };
    const userRole = loggedUser.role ? loggedUser.role.toLowerCase() : 'visitante';
    ...
    console.log(`📊 [reloadDataForWeek] Empleados cargados: ${state.trabajadores.length}`);
    
    // Reconstruir cache
    buildActivitiesCache();
    
    // Actualizar fecha de semana cargada
    state.lastLoadedWeekStart = fecha;
    
    return true;
  } catch (err) {
    console.error('❌ [reloadDataForWeek] Error:', err);
    return false;
  }
}
 ...
 * - 🔥 NUEVO: Recarga datos si cambiamos de semana
 */
function renderForCurrentState() {
  // [MODIFICADO] Calcular el índice del día considerando daysOffset
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const todayDayIndex = today.getDay();
  
  // Calcular la fecha real basada en daysOffset
  const displayedDate = new Date(today);
  displayedDate.setDate(today.getDate() + state.daysOffset);
  
  // 🔥 NUEVO: Verificar si necesitamos cargar datos de otra semana
  const displayedWeekStart = getWeekStartForDate(displayedDate);
  const needsReload = state.lastLoadedWeekStart && state.lastLoadedWeekStart !== displayedWeekStart;
  
  if (needsReload) {
    console.log(`📅 [renderForCurrentState] Cambio de semana detectado: ${state.lastLoadedWeekStart} → ${displayedWeekStart}`);
    // Recargar datos de la nueva semana y volver a renderizar
    reloadDataForWeek(displayedWeekStart).then(() => {
      renderForCurrentStateInternal(displayedDate);
    });
    return;
  }
  
  // Si no necesita recarga, renderizar normalmente
  renderForCurrentStateInternal(displayedDate);
}

/**
 * 🔥 NUEVO: renderForCurrentStateInternal(displayedDate)
 * Función interna que hace el render real después de verificar/cargar la semana correcta
 */
function renderForCurrentStateInternal(displayedDate) {
  // Obtener el nombre del día de la fecha calculada
  ...
  // 🔥 CRÍTICO: Las extras completadas y el cache ya fueron construidos en checkForUpdates()
  // NO reconstruir aquí para evitar duplicación de filas
  (async () => {
    try {
      // 🔥 Mostrar TODOS los trabajadores (sin paginación)
      const visibleTrabajadores = state.trabajadores;
      ...
      const now = new Date();
      const isToday = state.daysOffset === 0; // [MODIFICADO] isToday ahora se basa en daysOffset
      ...
      // KPI - Actualizar contador (extras ya están cargadas)
      updateTaskProgressWidget();
      ...
      // 🔥 MODIFICADO: Ocultar flecha hacia atrás cuando estamos en hoy (daysOffset === 0)
      if (DOM.prevDayBtn) DOM.prevDayBtn.classList.toggle('hidden', state.daysOffset <= 0);
    } catch (err) {
      console.error('❌ Error en renderForCurrentState:', err);
    }
  })();
}
...
function calculateEmployeePoints(empleado, dayName, now) {
  ...
  // 🔥 CORREGIDO: Calcular la fecha usando daysOffset en lugar de currentDayIndex
  const today = new Date();
  today.setHours(0, 0, 0, 0);
...
function buildActivitiesCache() {
            ...
            if (esLaOriginal) {
              ...
              if (!yaExiste) {
                entry.extrasCompletadas.push({
                  ...
                  originalTaskId: tarea.id,
                  nombre: nombreExtra
                });
                console.log(`🔵 [buildActivitiesCache] Extra completada: ${trab.nombre} completó "${nombreExtra}" como extra`);
              }
              encontrada = true;
              break;
            }
        ...
        if (esExtraConId) {
          ...
          for (const [key, entry] of activityMap) {
            ...
            if (tieneOriginal) {
              ...
              console.log(`🔵 [buildActivitiesCache] Tarea extra de ${trab.nombre} vinculada a original ${originalId}`);
              encontrada = true;
              break;
            }
          }
      ...
      for (const [key, entry] of activityMap) {
        ...
        if (tieneOriginal) {
          ...
          if (!yaExiste) {
            ...
            console.log(`🔵 [buildActivitiesCache] Extra del histórico: usuario ${usuario ? usuario.nombre : idUsuarioQuePinto} completó tarea "${nombreExtra}" (ID original ${originalId})`);
            encontrada = true;
          }
          break;
        }
  ...
  for (const d of diasSemana) {
    ...
    // Convertir el mapa a array, duplicando instancias según número de empleados
    for (const [key, entry] of activityMap) {
      const numEmpleados = entry.empIds.size;
...
function buildRows(rowsData, visibleTrabajadores, isToday, now) {
  ...
  // 🔥 CORREGIDO: Usar daysOffset para calcular la fecha mostrada
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  ...
  // 🔥 CORREGIDO: isFutureDay se basa en daysOffset (si es > 0, es un día futuro)
  const isFutureDay = state.daysOffset > 0;
    ...
    visibleTrabajadores.forEach(trab => {
      ...
      if (tarea) {
        ...
        if (tarea.estatus === 5) {
          ...
          // 🔥 CORREGIDO: Usar daysOffset para determinar si es hoy
          const isDisplayedDateToday = (state.daysOffset === 0);
        ...
        // PERO solo mostrar si estamos viendo el día de HOY (state.daysOffset === 0)
        const extraCompletadaPorEste = isToday ? (rowData.extrasCompletadas || []).find(e => 
        ...
        if (extraCompletadaPorEste) {
          ...
          console.log(`🔵 [buildRows] Celda azul fuerte para ${trab.nombre} - completó extra de tarea original`);
        } else {
          // 🔥 Solo mostrar tareas vencidas/disponibles si estamos viendo HOY
          if (!isToday) {
            // Día futuro o pasado - celda vacía
            td.textContent = '-';
            tr.appendChild(td);
            return;
          }
          ...
          // 🔥 PASO 2: Verificar si la tarea aún está activa en tareas_extra (no expiró por TTL)
          let tareaExtraActiva = false;
          if (tareaVencida && empleadoOriginal) {
            tareaExtraActiva = tareasExtraActivas.some(te => 
              String(te.TaskID) === String(tareaVencida.id)
            );
          }
          ...
          // Si encontramos una tarea vencida, está ACTIVA (TTL válido), NADIE la completó como extra Y NO es el empleado original → mostrar disponible (azul claro)
...
function showCellMenu(evt, td) {
  ...
  if (td.dataset.tareaId) menuContext.tareaId = Number(td.dataset.tareaId);
 ...
 * - Es el día de hoy Y no ha llegado la hora de la siguiente tarea
...
function validarSiPuedeCompletarse(tarea, empId, diaName) {
  ...
  // Si no es hoy, permitir completar
  if (!isToday) {
  ...
  // Obtener todas las tareas del empleado en ese día
...
async function completarTareaNormalConPin(taskInfo, empleadoId, username) {
  try {
    ...
    const fullResp = await fetch('/empleados-con-tareas', { cache: 'no-store' });
    if (!fullResp.ok) {
      throw new Error('No se pudo obtener empleados después de guardar');
    }
    
    const parsed = await fullResp.json();
...
async function completarTareaExtraConPin(taskInfo, empleadoId, username) {
  try {
    ...
    if (!empleadoOriginal) {
      ...
      console.error('   originalEmpId buscado:', taskInfo.originalEmpId);
      console.error('   IDs disponibles:', state.trabajadores.map(t => t.id));
      throw new Error('Tarea original no encontrada');
    }
    
    console.log('✅ Empleado original encontrado:', empleadoOriginal.nombre, 'ID:', empleadoOriginal.id);
    console.log('   Buscando tarea ID:', taskInfo.originalTaskId, 'en día:', dayName);
    
    const tareasOriginal = (empleadoOriginal.tareas_asignadas && empleadoOriginal.tareas_asignadas[dayName]) || [];
    console.log('   Tareas disponibles en ese día:', tareasOriginal.map(t => ({id: t.id, nombre: t.nombre})));
    
    let tareaOriginal = tareasOriginal.find(t => String(t.id) === String(taskInfo.originalTaskId));
    
    // 🔥 SI NO ENCONTRAMOS EN tareas_asignadas, buscar en tareasExtraActivas
    // (importante para visitantes que no tienen todas las tareas cargadas)
    if (!tareaOriginal && tareasExtraActivas && tareasExtraActivas.length > 0) {
      console.log('   ⚠️ No encontrada en tareas_asignadas, buscando en tareasExtraActivas...');
      const extraActiva = tareasExtraActivas.find(te => String(te.TaskID) === String(taskInfo.originalTaskId));
      if (extraActiva) {
        console.log('   ✅ Encontrada en tareasExtraActivas:', extraActiva.Nombre);
        // 🔥 IMPORTANTE: Obtener el nombre REAL de la tarea desde taskInfo (que viene del HTML/dataset)
        // NO usar extraActiva.Nombre que es genérico "Tarea Extra"
        const nombreReal = taskInfo.taskDesc || taskInfo.nombre || extraActiva.Nombre;
        console.log('   📝 Usando nombre real de la tarea:', nombreReal);
        // Construir objeto compatible
        tareaOriginal = {
          id: extraActiva.TaskID,
          nombre: nombreReal,
          descripcion: nombreReal,
          hora: extraActiva.Hora,
          hora_fin: extraActiva.HoraFin || '',
          puntaje: extraActiva.Puntaje
        };
      }
    }
    
    if (!tareaOriginal) {
      console.error('❌ Tarea original no encontrada en ningún lado');
      console.error('   Buscaba: ID=' + taskInfo.originalTaskId);
      console.error('   En:', empleadoOriginal.nombre);
      console.error('   Tareas en tareas_asignadas:', tareasOriginal.map(t => t.id));
      console.error('   Tareas extra activas:', (tareasExtraActivas || []).map(t => t.TaskID));
      throw new Error('Tarea original no encontrada');
    }
    ...
    if (!empleadoQueCompleta.tareas_asignadas[dayName]) {
      empleadoQueCompleta.tareas_asignadas[dayName] = [];
    }
    
    const tareasDelQueCompleta = empleadoQueCompleta.tareas_asignadas[dayName];
    ...
    // Agregar la tarea extra al empleado que la completó
    tareasDelQueCompleta.push(nuevaTareaExtra);
    
    // Enviar actualización al backend (empleado que COMPLETÓ)
    const response = await fetch(`/empleados/${empleadoId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        tareas_asignadas: {
          [dayName]: tareasDelQueCompleta
        }
      })
    });
    ...
    // 🔥 CRÍTICO: Guardar en tareas_extras_historico (igual que GUI y Reloj)
    // Esto asegura que persista en BD y se muestre automáticamente en recargas
    try {
      const historico_resp = await fetch('/tareas-extras/guardar-historico', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          id_usuario_asignada: empleadoId,
          tarea_original_id: taskInfo.originalTaskId,
          nombre: tareaOriginal.nombre || taskInfo.taskDesc,
          hora_origen: tareaOriginal.hora || taskInfo.hour,
          puntaje_original: tareaOriginal.puntaje || 0,
          id_usuario_origen: taskInfo.originalEmpId,
          fecha: new Date().toISOString().split('T')[0]
        })
      });
      
      if (historico_resp.ok) {
        console.log('✅ [PIN] Tarea extra guardada en histórico de BD');
      } else {
        console.warn('⚠️ [PIN] No se guardó en histórico, pero la tarea está en tareas_asignadas');
      }
    } catch (err) {
      console.warn('⚠️ [PIN] Error guardando en histórico (no crítico):', err);
    }
    
    // 🔥 CRÍTICO: Eliminar de tareasExtraActivas (igual que Reloj)
    // Esto saca la tarea de la lista de "extras disponibles para completar"
    // para que no vuelva a aparecer en el reloj ni en la GUI
    try {
      const eliminar_resp = await fetch('/tareas-extra/eliminar', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          task_id: String(taskInfo.originalTaskId),
          nombre: tareaOriginal.nombre || taskInfo.taskDesc,
          completed_by_id: empleadoId,
          original_emp_id: taskInfo.originalEmpId,
          puntaje: tareaOriginal.puntaje || 0,
          hora: tareaOriginal.hora || taskInfo.hour,
          descripcion: tareaOriginal.descripcion || '',
          skip_historico: true  // 🔥 YA se guardó arriba, no duplicar
        })
      });
      
      if (eliminar_resp.ok) {
        console.log('✅ [PIN] Tarea extra eliminada de tareasExtraActivas');
      } else {
        console.warn('⚠️ [PIN] Error eliminando de extras activas:', await eliminar_resp.text());
      }
    } catch (err) {
      console.warn('⚠️ [PIN] Error eliminando de extras activas (no crítico):', err);
    }
    ...
    // Recargar empleados actualizado
    const resp = await fetch('/empleados-con-tareas', { cache: 'no-store' });
    ...
    // 🔥 CRÍTICO: Cargar extras completadas ANTES de renderizar (igual que GUI y Reloj)
    await cargarTareasExtrasCompletadas();
    
    buildActivitiesCache();
    
    // Refrescar tareas extra disponibles (forzar después de completar una extra)
    await refreshAvailableExtraTasks(true);
...
async function deleteTask(tarea) {
  ...
  try {
    ...
    // 🔥 Forzar reload completo de la página para limpiar cache
    setTimeout(() => {
      location.reload();
    }, 2000);
  ...
  (async () => {
    try {
      ...
      try {
        ...
        showToast('Cambio guardado correctamente. Recargando...', 'success', 2800);
        
        // 🔥 Forzar reload completo de la página después de actualizar tarea
        setTimeout(() => {
          location.reload();
        }, 2000);
      } catch (e) {
...
function updateClockVisibility() {
  // [MODIFICADO] Usar daysOffset en lugar de currentDayIndex para saber si estamos en hoy
...
function updateTime() {
  ...
  if (state.currentDayIndex === now.getDay() && state.lastMinuteScrolled !== now.getMinutes()) {
    ...
    centerOnCurrentTime({ forceScroll: true, now });
    adjustCenterBandHeight();
    // Cargar extras ANTES de actualizar contador
    (async () => { await cargarTareasExtrasCompletadas(); updateTaskProgressWidget(); })();
...
function updateTaskProgressWidget() {
  ...
  empleados.forEach(emp => {
    ...
    tareasDia.forEach(t => {
      ...
      // 🔥 EXTRAS (estatus 5): NO las sumamos aquí
      // Se suman DESPUÉS desde el histórico para evitar duplicados
      if (t.estatus === 5) {
        console.log(`  ➕ EXTRA DETECTADA: se sumará desde histórico (no aquí para evitar duplicados)`);
        return; // No procesar más, las extras se cuentan desde el histórico
      }
  ...
  if (Array.isArray(tareasExtrasCompletadas)) {
    ...
    tareasExtrasCompletadas.forEach(extra => {
      ...
      // 🔥 SIMPLE: Las tareas del histórico son la ÚNICA fuente de verdad para extras completadas
      // No hay duplicación porque ya lo filtramos en la BD
      // Solo contar si tienen puntaje > 0
      if (puntaje > 0) {
